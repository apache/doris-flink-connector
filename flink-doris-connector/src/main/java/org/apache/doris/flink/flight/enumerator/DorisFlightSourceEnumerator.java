...
package org.apache.doris.flink.flight.enumerator;
 ...
 */
public class DorisFlightSourceEnumerator
        implements SplitEnumerator<DorisFlightSourceSplit, PendingSplitsCheckpoint> {

    private static final Logger LOG = LoggerFactory.getLogger(DorisFlightSourceEnumerator.class);
    private final SplitEnumeratorContext<DorisFlightSourceSplit> context;

    private final FlightSplitAssigner splitAssigner;

    public DorisFlightSourceEnumerator(
            SplitEnumeratorContext<DorisFlightSourceSplit> context,
            FlightSplitAssigner splitAssigner) {
    ...
    public void handleSplitRequest(int subtaskId, @Nullable String hostname) {
        ...
        final Optional<DorisFlightSourceSplit> nextSplit = splitAssigner.getNext(hostname);
        if (nextSplit.isPresent()) {
            final DorisFlightSourceSplit split = nextSplit.get();
    ...
    @Override
    public void addSplitsBack(List<DorisFlightSourceSplit> splits, int subtaskId) {
